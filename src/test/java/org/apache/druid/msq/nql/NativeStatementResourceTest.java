...
public class NativeStatementResourceTest extends NativeMSQTestBase
{
  ...
  private Map<String, Object> getTaskReportAsMap(final String queryId) throws JsonProcessingException
  {
    return objectMapper.readValue(
        objectMapper.writeValueAsString(TaskReport.buildTaskReports(selectTaskReport(queryId))),
  ...
  private void setupMocks(OverlordClient indexingServiceClient) throws JsonProcessingException
  {
    ...
    Mockito.when(indexingServiceClient.taskReportAsMap(FINISHED_SELECT_MSQ_QUERY))
           .thenReturn(Futures.immediateFuture(getTaskReportAsMap(FINISHED_SELECT_MSQ_QUERY)));
  ...
  public void testFinishedSelectMSQQuery() throws Exception
  {
    ...
    Assert.assertEquals(Response.Status.OK.getStatusCode(), response.getStatus());
    Assert.assertEquals(objectMapper.writeValueAsString(new NativeStatementResult(
        FINISHED_SELECT_MSQ_QUERY,
